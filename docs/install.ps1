# Elitzur Games — Voice Chat auto-installer (Windows)
# Installs Fabric + Fabric API + Simple Voice Chat for MC 26.1.2
# Run from PowerShell:
...
Write-Host " Elitzur Games — Voice Chat Installer" -ForegroundColor Yellow
Write-Host "==========================================" -ForegroundColor Yellow
...
# 2. Ensure .minecraft + mods folder
...
# 4b. Create launcher profile manually (since we used -noprofile to avoid older UIs)
...
if (Test-Path $profilesPath) {
    try {
        ...
        if ($fabricVer) {
            ...
            $profiles.profiles | Add-Member -NotePropertyName $profileId -NotePropertyValue $newProfile -Force
...
Write-Host "2. בחר את הפרופיל ' Elitzur Games (Fabric $MC_VERSION)' (או fabric-loader-$MC_VERSION)"
Write-Host "3. לחץ Play"
Write-Host "4. התחבר לשרת — תראה אייקון מיקרופון בפינת המסך"
